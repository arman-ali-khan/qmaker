/*
  ...
  2. Security
    ...
    - School owners and teachers can manage their school's question papers
    - Maintain data isolation between schools

  3. Structure
    - Question papers with metadata (title, class, subject, etc.)
    - Questions with Bengali text support
...
CREATE TABLE IF NOT EXISTS question_papers (
  ...
  watermark text,
  page_margin integer DEFAULT 20, -- in pixels
  header_text text,
  footer_text text,
...
CREATE POLICY "School users can view their school question papers"
  ...
  USING (
    EXISTS (
      ...
      AND users.school_id = question_papers.school_id
      AND users.role IN ('school_owner', 'teacher')
    )
  );

CREATE POLICY "School users can create question papers"
...
CREATE POLICY "School users can update their question papers"
  ...
  USING (
    ...
    AND (auth.uid() = created_by OR EXISTS (
      SELECT 1 FROM users
      WHERE users.id = auth.uid()
      AND users.role = 'school_owner'
    ))
  );

CREATE POLICY "School users can delete their question papers"
...
CREATE POLICY "School users can manage question options"
  ...
  USING (
    EXISTS (
      ...
      JOIN question_papers qp ON qp.id = q.paper_id
      JOIN users u ON u.school_id = qp.school_id
